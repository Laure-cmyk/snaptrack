...
function saveLocationEdit() {
  if (editingLocationIndex.value !== null && editingLocationDescription.value.trim()) {
    localTrail.value.locations[editingLocationIndex.value].description =
      editingLocationDescription.value.trim();
    emitUpdate();
  }
  closeEditDialog();
}
...
<template>
  <v-form ref="form" v-model="valid" class="pa-6 bg-grey-lighten-4">
    ...
    <v-textarea
      v-model="localTrail.description"
    ...
    <!-- Empty State -->
    <div
      v-if="localTrail.locations.length === 0"
      class="text-center py-6 mb-6 bg-grey-lighten-3"
    ...
    <v-list v-else class="mb-6 pa-0" style="border-radius: 8px">
      <v-list-item
        v-for="(location, index) in localTrail.locations"
        :key="location.id"
    ...
    <v-dialog v-model="editLocationDialog" max-width="500">
      <v-card rounded="xl" class="pa-4">
        ...
        <v-card-text class="px-6 py-4">
          ...
          <v-textarea
            v-model="editingLocationDescription"
            placeholder="Modifiez l'énigme"
            variant="outlined"
            rows="3"
            bg-color="grey-lighten-3"
          />
