  ...
  try {
    ...
    if (err.name === 'NotAllowedError' || err.name === 'PermissionDeniedError') {
      ...
      permissionMessage.value =
        "L'accès à la caméra est nécessaire pour prendre une photo du lieu. Veuillez autoriser l'accès pour continuer.";
...
function handleGPSError(err) {
  const errorMessages = {
    1: {
      ...
      type: 'gps',
      message:
        "L'accès à votre localisation est nécessaire pour enregistrer la position du lieu. Veuillez autoriser l'accès pour continuer.",
      showDialog: true
    },
...
<template>
  <v-form ref="form" v-model="valid" class="pa-6 bg-grey-lighten-4">
    ...
    <div class="mb-6">
      ...
      <!-- Photo Field -->
      <div
        v-if="!localLocation.image"
        class="photo-field bg-grey-lighten-3 d-flex align-center justify-center"
      ...
      <div v-else class="position-relative">
        ...
        <v-btn
          ...
          style="top: 12px; right: 12px; background-color: rgba(0, 0, 0, 0.5)"
          @click="removePhoto"
        >
    ...
    <v-textarea
      ...
      placeholder="Ajoutez une énigme pour trouver ce lieu"
      variant="outlined"
      rows="3"
    ...
    <div v-if="localLocation.coordinates" class="text-caption text-grey mb-6 d-flex align-center">
      ...
      Position: {{ localLocation.coordinates.lat.toFixed(6) }},
      {{ localLocation.coordinates.lng.toFixed(6) }}
    </div>
    ...
    <v-dialog v-model="permissionDialog" max-width="400" persistent>
      <v-card rounded="xl" class="pa-4">
        <v-card-title class="text-h6 font-weight-bold text-center pa-4">
          <v-icon
            size="48"
            :color="permissionType === 'camera' ? 'indigo-darken-1' : 'orange-darken-1'"
            class="mb-2"
          >
